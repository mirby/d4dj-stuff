function English(str) {
		switch (str) {
			...
			case 24:{ //When doing rehearsals, do them in reverse order so it's easier to fix if you make a mistake.
				return "When doing rehearsals, do them in reverse order so it's easier to fix if you make a mistake."
			}break;
			case "%INITIAL%":{ //This message is the first thing to display for a normal calculation.
				...
				*/
				...
				return "Calculating from %START% to %TARGET% for event type %EVENT%...\n\t(All games are done in Free Live AND Full Combo on Manual Mode)\n\nFound a park! %STEPS% step%PLURAL_STEPS% and %VOLTAGE% voltage required!"
			}break;
